...
axes = axes.flatten()
fig.suptitle("Normalized Infection in the Workplace\n" + 
             "50% population masked and social distancing\n" + 
             "50% reduction of both, each workplace 100% vaccinated")
...
curves = sorted(curves, key=lambda x: x[0])
...
for c in curves:
    ...
    for age in range(19):
        ...
        sir = sir_age[age] 
        I = sir['I'] / N_age[age]
        ax.plot(sir['t'], I, label="%2d" % nb_top_wk)
        ax.set_title("k=%2d" % age, fontsize=6)
        ax.set_xlabel("Time", fontsize=5)
        ax.set_ylabel("Normalized Infections", fontsize=5)
        ax.set_xlim(0, 100)
        ax.set_ylim(0, 0.4)
