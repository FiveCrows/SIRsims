...
class TransmissionWeighter:
    ...
    def getWeight(self, personA, personB, environment):
        ...
        if environment.preventions != None:
            ...
            # c * weight
            weight = weight * (
                        1 - (1 - self.prevention_reductions["distancing"]) * environment.preventions["distancing"])
...
class PopulaceGraph:
    ...
    def __init__(self, partition, graph=None, populace=None,
        ...
        if partition != None:
            ...
            for index in workplaces:
                if index != None:
                    workplace = PartitionedEnvironment(index, workplaces[index], "workplace", self.populace,
                                                       work_matrices[index], partition)
    ...
    def simulate(self, gamma, tau, simAlg=EoN.fast_SIR, title=None, full_data=True):
        start = time.time()
        simResult = simAlg(self.graph, tau, gamma, rho=0.001, transmission_weight='transmission_weight',
    ...
    def returnContactMatrix(self, environment):
        ...
        for i in graph.nodes():
            ...
            for j in contacts:
                ...
                contact_matrix[iPartition, jPartition] += self.graph[i][j]['transmission_weight'] / partition_sizes[
                    iPartition]
        ...
        for i in range(lo, hi):
            for j in range(lo, i + 1):
                ...
                while True:
                    ...
                    # stop when desired number of edges is reached                    lg = len(ddict)
                    if lg == Nij: break
