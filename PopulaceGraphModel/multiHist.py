...
#for the age plot
agePartition = partitioner.partitionGroup(range(len(model.populace)), model.populace)

fig,axs = plt.subplots(4)
plt.ylabel('count')
axs[0].set_title("household size")
axs[1].set_title("school size")
axs[2].set_title("log-log workplace size")
axs[3].set_title("population age distribution")
axs[0].bar(range(1,len(h_sizes)),h_sizes[1:])
axs[1].bar(range(len(s_sizes)),s_sizes)
axs[1].set_xticklabels(np.arange(0,100*(40),100))
axs[3].set_xticklabels([0] + (partitioner.labels[::2]))
axs[2].bar(1+np.asarray(range(len(w_sizes))),np.log(w_sizes))
axs[3].bar(range(len(agePartition)), [len(agePartition[key]) for key in agePartition])
plt.ylabel('count')
plt.tight_layout()
