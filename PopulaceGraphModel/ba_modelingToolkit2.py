...
class Environment:
    ...
    def reweight(self, netBuilder, newPreventions=None):
        print("Reweight should not be called");
        quit()
        """
...
class NetBuilder:
    ...
    def getWeight(self, personA, personB, environment):
        print("getWeight should not be called");
        quit()
        """
        ...
        isDistanced = int(
            bool(environment.distancing_adoption[personA]) or bool(environment.distancing_adoption[personB]))
...
class PopulaceGraph:
    ...
    def __init__(self, partitioner, prevention_adoptions=None, prevention_efficacies=None,
                 attributes=['sp_hh_id', 'work_id', 'school_id', 'race', 'age'], slim=False, timestamp=None):
        """
...
IndexError: index 60 is out of bounds for axis 0 with size 60
    ...
    def rank_workplaces(self):
        ...
        self.cum_sum_work_pop = np.cumsum(
            self.ordered_work_pop[1:])  # remove the first work which are the people with no workplace
    ...
    def simulate(self, gamma, tau, simAlg=EoN.fast_SIR, title=None, full_data=True, global_dict={}):
        ...
        simResult = simAlg(self.graph, tau, gamma, initial_recovereds=self.initial_recovered,
...
class Record:
    ...
    def printGraphStats(self, graph, statAlgs):
        if not nx.is_connected(graph):
            ...
            self.print("{} of nodes lie within the maximal subgraph".format(
                max_subgraph.number_of_nodes() / graph.number_of_nodes()))
        else:
            max_subgraph = graph
        graphStats = {}
